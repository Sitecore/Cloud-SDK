...
describe('my-app', () => {
  it('should initialize CloudSDK with events, personalize and search and call the related functions ', () => {
    ...
    cy.getCookie('sc_83d8199c-2837-4c29-a8ab-1bf234fea2d1').then((cookie) =>
      expect(cookie?.value).to.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i)
    );
    ...
    cy.wait(1000);
    cy.get('[data-testid="sendGetPageWidgetDataBtn"]').click();
    cy.wait('@searchRequest').then((request) => {
      expect(request.request.body.context.page.uri).to.eq('/test');
    });

    cy.wait(1000);
    cy.get('[data-testid="sendSearchEntityViewBtn"]').click();
    cy.wait('@eventRequest')
      .then((request) => {
        expect(request.request.body.sc_search.data.value.context.page.uri).to.eq(
          'https://www.sitecore.com/products/content-cloud'
        );
      })
      .its('response.statusCode')
      .should('eq', 201);
