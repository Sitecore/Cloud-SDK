Feature: Init CloudSDK server-side
    ...
    Scenario: Developer calls new init from Middleware without addSearch and tries to send an event
        ...
        Then an error is thrown: '[IE-0019] You must first initialize the Cloud SDK and the "search" package. First, import "CloudSDK" from "@sitecore-cloudsdk/core/server", and import "@sitecore-cloudsdk/search/server". Then, run "await CloudSDK().addEvents().addSearch().initialize()".'
