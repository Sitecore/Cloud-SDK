...
export async function POST(req: NextRequest) {
  ...
  if (type === 'fetch') {
    const dataFilePath = path.resolve(path.join('..', 'events-e2e', 'src', 'fixtures', 'local', 'fetchData.json'));
    ...
    const dataFilePath = path.resolve(path.join('..', 'events-e2e', 'src', 'fixtures', 'local', 'logsData.json'));
