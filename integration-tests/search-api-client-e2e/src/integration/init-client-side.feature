...
Feature: Init CloudSDK client-side
    ...
    Scenario: Init CloudSDK without addSearch and send a search event
        ...
        Then an error is thrown: '[IE-0018] You must first initialize the Cloud SDK and the "search-api-client" package. First, import "CloudSDK" from "@sitecore-cloudsdk/core/browser", and import "@sitecore-cloudsdk/search-api-client/browser". Then, run "CloudSDK().addEvents().addSearch().initialize()".'
