...
Scenario Outline: Developer sends search widget suggestion click event from browser with a valid payload
    Given the '/send-widget-suggestion-click-event' page is loaded
    ...
    """
     ...
     """
    And the 'sendWidgetSuggestionClickEvent' button is clicked
...
Scenario: Developer sends widget suggestion click event from Middleware with a valid payload
    Given the '/send-widget-suggestion-click-event' page is loaded with 'testID' name and 'sendWidgetSuggestionClickEventFromMiddleware' value query parameter
    Then the request with id 'sendWidgetSuggestionClickEventFromMiddleware' will contain:
    """
        "sc_search":{"data":{"action_cause":"suggestion","value":{"context":{"page":{"uri":"https://www.sitecore.com/products/content-cloud"}},"filters":[{"display_name":["test"],"name":"test","title":"test","value":["test"],"value_position":[1]}],"request":{"advanced_query_text":"test1","keyword":"test_keyword","modified_keyword":"test2","num_requested":20,"num_results":10,"page_number":2,"page_size":1,"redirect_url":"test3","total_results":10},"rfk_id":"12345"}},"metadata":{"ut_api_version":"1.0"}
    """
